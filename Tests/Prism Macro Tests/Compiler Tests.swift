...
private struct `Prism accessibility compiler contracts` {
    ...
    func `public consumers preserve syntax access and ownership boundaries`() throws {
        ...
        for _ in 0..<12 {
            if files.fileExists(atPath: products.appendingPathComponent("Prism_Macro.swiftmodule").path)
                || files.fileExists(atPath: products.appendingPathComponent("Modules/Prism_Macro.swiftmodule").path)
            {
                break
            }
            products.deleteLastPathComponent()
        }
        let plugin = ["Prism Macro Plugin", "Prism Macro Plugin-tool"]
        ...
        let arguments = [
            ...
            "-I", scratch.path,
            "-load-plugin-executable", executable.path + "#Prism_Macro_Plugin",
        ]
